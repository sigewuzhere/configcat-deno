import { IConfigCatLogger } from "./index.ts";
import {
  ProjectConfig,
  Setting,
  RolloutRules,
...
export interface IRolloutEvaluator {
  Evaluate(
    ...
    defaultValue: any,
    user?: User,
    defaultVariationId?: any,
  ): ValueAndVariationId | undefined | null;
}
...
export class RolloutEvaluator implements IRolloutEvaluator {
  ...
  Evaluate(
    ...
    if (!config || !config.ConfigJSON) {
      this.logger.error(
        "JSONConfig is not present. Returning default value: '" + defaultValue +
          "'.",
      );
    ...
    if (!config.ConfigJSON[key]) {
      let s: string = "Evaluating getValue('" + key +
        "') failed. Returning default value: '" + defaultValue + "'.";
      s += " Here are the available keys: {" +
        Object.keys(config.ConfigJSON).join() + "}.";
    ...
    if (user) {
      result = this.EvaluateRules(
        config.ConfigJSON[key][Setting.RolloutRules],
        user,
        eLog,
      );

      if (result.ValueAndVariationId == null) {
        result.ValueAndVariationId = this.EvaluateVariations(
          config.ConfigJSON[key][Setting.RolloutPercentageItems],
          key,
          user,
        );
        if (result.ValueAndVariationId && result.EvaluateLog) {
          result.EvaluateLog.ReturnValue = result.ValueAndVariationId.Value;
        }
        if (
          config.ConfigJSON[key][Setting.RolloutPercentageItems].length > 0 &&
          result.EvaluateLog
        ) {
          result.EvaluateLog.OpAppendLine(
            "Evaluating % options => " +
      ...
      if (
        (config.ConfigJSON[key][Setting.RolloutRules] &&
          config.ConfigJSON[key][Setting.RolloutRules].length > 0) ||
        (config.ConfigJSON[key][Setting.RolloutPercentageItems] &&
          config.ConfigJSON[key][Setting.RolloutPercentageItems].length > 0)
      ) {
        let s: string = "Evaluating getValue('" + key + "'). ";
        s +=
          "UserObject missing! You should pass a UserObject to getValue(), in order to make targeting work properly. ";
    ...
    if (result.ValueAndVariationId == null && result.EvaluateLog) {
  ...
  private EvaluateRules(
    ...
    let result: EvaluateResult = new EvaluateResult();
    result.ValueAndVariationId = undefined;

    if (rolloutRules && rolloutRules.length > 0) {
      for (let i: number = 0; i < rolloutRules.length; i++) {
        let rule: any = rolloutRules[i];

        let comparisonAttribute: string | undefined = this.GetUserAttribute(
          user,
          rule[RolloutRules.ComparisonAttribute],
        );
        ...
        let log: string = "Evaluating rule: '" + comparisonAttribute + "' " +
          this.RuleToString(comparator) + " '" + comparisonValue + "' => ";
